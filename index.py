...
# Adicione uma nova coluna para os valores convertidos em euros
df_main['VALOR REVENDA (€/L)'] = None

# Defina a taxa de câmbio (substitua pela taxa de câmbio atual)
taxa_cambio = 0.19  # Exemplo, substitua pela taxa de câmbio atual

# Converta os valores para euros
df_main['VALOR REVENDA (€/L)'] = df_main['VALOR REVENDA (R$/L)'] * taxa_cambio
...
def func(data, toggle):
    ...
    max = dff.groupby(['ANO'])['VALOR REVENDA (€/L)'].max()
...
def graph1(data, ano, regiao, toggle):
    ...
    dff_regiao = df_filtered.groupby(['ANO', 'REGIÃO'])['VALOR REVENDA (€/L)'].mean().reset_index()
    dff_estado = df_filtered.groupby(['ANO', 'ESTADO', 'REGIÃO'])['VALOR REVENDA (€/L)'].mean().reset_index()
...
def range_slider(range, data):
    dff = pd.DataFrame(data)
    start_date = pd.to_datetime(f'{range[0]}-01-01')
    end_date = pd.to_datetime(f'{range[1]}-12-31')
    dff = dff[(dff['ANO'] >= start_date) & (dff['ANO'] <= end_date)]
